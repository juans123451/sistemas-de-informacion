...
// Mostrar la zona horaria actual y la fecha/hora
...
function generarDiagramaProductosVendidos() {
    $sentencia = "
        SELECT 
...
function calcularGananciaProductos() {
    $sentencia = "
        SELECT 
            IFNULL(SUM((pv.precio - p.compra) * pv.cantidad), 0) AS total_ganancias
        FROM 
            productos_ventas pv
        JOIN 
            productos p
    ...
    ";
    $fila = select($sentencia);
    if ($fila) return $fila[0]->total_ganancias;
